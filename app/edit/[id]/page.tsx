...
import { calculateWalletYield, calculateCEXYield, calculateBankYield, getExchangeRateAsync, normalizeBankAssets } from '@/lib/data';
import { NetWorthRecord, OnChainAsset, CEXAsset, BankAsset, BankPosition } from '@/types';
...
export default function EditRecord() {
        ...
        if (response.ok) {
          ...
          if (foundRecord) {
            ...
            const migratedRecord = {
              ...
              bankAssets: normalizeBankAssets(foundRecord.bankAssets)
            };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const bankTotal = record.bankAssets.reduce((sum, asset) => sum + asset.totalValueUSD, 0);
              ...
              <div>
                ...
                <div className="mt-1 px-3 py-2 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md text-sm font-medium text-gray-900 dark:text-white">
                  ${(() => {
                    ...
                    const bankTotal = ((Array.isArray(record.bankAssets) ? record.bankAssets : Object.values(record.bankAssets || {})).reduce((sum, asset) => sum + ((asset as any).totalValueUSD ?? 0), 0)) as number;
                    ...
                    <div key={position.id} className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-3 p-3 bg-gray-50 dark:bg-gray-700 rounded">
                      ...
                      <div className="flex items-end justify-between gap-2">
                        ...
                        <button
                          ...
                          className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 text-xs mb-2"
                        >
                          <Trash2 className="w-3 h-3" />
                        </button>
                      </div>
