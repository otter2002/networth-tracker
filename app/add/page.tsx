                    ...
                    <div className="mt-3 p-3 bg-green-50 dark:bg-green-900/20 rounded">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-xs">
                        ...
                        <div>
                          <span className="text-gray-600 dark:text-gray-400">生息价值:</span>
                          <span className="ml-1 font-medium">${(() => {
                            const {calculateCEXYield} = require('@/lib/data');
                            return calculateCEXYield(asset).yieldValueUSD.toFixed(2);
                          })()}</span>
                        </div>
                        <div>
                          <span className="text-gray-600 dark:text-gray-400">总APR:</span>
                          <span className="ml-1 font-medium">{(() => {
                            const {calculateCEXYield} = require('@/lib/data');
                            return calculateCEXYield(asset).totalAPR.toFixed(2);
                          })()}%</span>
                        </div>
                        <div>
                          <span className="text-gray-600 dark:text-gray-400">日收入:</span>
                          <span className="ml-1 font-medium">${(() => {
                            const {calculateCEXYield} = require('@/lib/data');
                            return calculateCEXYield(asset).dailyIncome.toFixed(2);
                          })()}</span>
                        </div>
                        <div>
                          <span className="text-gray-600 dark:text-gray-400">年收入:</span>
                          <span className="ml-1 font-medium">${(() => {
                            const {calculateCEXYield} = require('@/lib/data');
                            return calculateCEXYield(asset).yearlyIncome.toFixed(2);
                          })()}</span>
                        </div>
                      </div>
                    </div>
