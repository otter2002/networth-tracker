...
const pool = createPool({
  connectionString: 'postgres://neondb_owner:npg_fp6QFIUbgSx9@ep-twilight-bonus-a1qwiq71-pooler.ap-southeast-1.aws.neon.tech/neondb?sslmode=require'
});
...
async function importHistoricalDataViaSQL() {
  try {
    ...
    for (const record of historicalData) {
      ...
      const result = await pool.sql`
        ...
        VALUES (
          ...
          ${JSON.stringify(record.onChainAssets)},
          ${JSON.stringify(record.cexAssets)},
          ${JSON.stringify(record.bankAssets)}
        )
        RETURNING id
      `;
      
      console.log(`✅ ${record.date} 数据导入成功，记录ID: ${result.rows[0].id}`);
    }
